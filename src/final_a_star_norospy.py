    ...
    def find_next_point(self, fill):
        ...
        for i in range(pix_num):
            for j in range(pix_num):
                if self.img[i, j] >= 250:
                    val_neg_y = np.append(self.pix_neg_y, self.img[
                        i - 1, j])  # looks in negative 'y' direction of .pgm
                    val_pos_y = np.append(self.pix_pos_y, self.img[
                        i + 1, j])  # Looks for walls in the positive 'y' direction of .pgm
                    val_pos_x = np.append(self.pix_pos_x, self.img[
                        i, j + 1])  # Looks for walls in the positive 'x' direction of .pgm
                    val_neg_x = np.append(self.pix_neg_x, self.img[
                        i, j - 1])  # Looks for walls in the negative 'x' direction of .pgm
                    if val_neg_y == 205:
                    ...
                    if val_pos_x == 205:
                        self.border_x = np.append(self.border_x, j)
                        self.border_y = np.append(self.border_y, i)
        plt.scatter(self.border_x, self.border_y, color='black',
                    marker='.')  ### <------------ next spot
...
class floodFill():
    ...
    def add_to_queue(self, pixel_location, pixel_weight, dista):
        ...
        self.priQueue.insert(
            Node(np.append(pixel_location, [round(dista, 2)]).tolist(), pixel_weight))
    ...
    def flood_fill_do(self, pixel_loc_in, end_pixel_loc_in):
            ...
            for i in range(len(neighbors)):
                ...
                if closed_array[int(neighbors[i][0])][int(neighbors[i][1])] < 1:
                    dist = queue_val[0][2] + np.sqrt(
                        (neighbors[i][0] - queue_val[0][0]) ** 2 + (
                                    neighbors[i][1] - queue_val[0][1]) ** 2)
        ...
        def check_for_obstical(y, x, width, offset, x_star, y_star):
            for j in range(width):
                val_neg_y = np.append(pix_val_left, updated_image[
                    x - j, y])  # looks in negative 'y' direction of .pgm
                val_pos_y = np.append(pix_val_right,
                                      updated_image[
                                          x + j, y])  # Looks for walls in the positive 'y' direction of .pgm
                val_pos_x = np.append(pix_val_up,
                                      updated_image[
                                          x, y + j])  # Looks for walls in the positive 'x' direction of .pgm
                val_neg_x = np.append(pix_val_down,
                                      updated_image[
                                          x, y - j])  # Looks for walls in the negative 'x' direction of .pgm
                if val_neg_y == 0:
        ...
        for i in path_length:
            ...
            if pix_dist >= pix_expand:
                ...
                prev_count = count
                self.x_star, self.y_star = check_for_obstical(y_vals[i], x_vals[i], width, offset,
...
if __name__ == '__main__':
    ...
    val_out = cProfile.run('fill.flood_fill_do(tuple([-3.4, -3.4]), tuple([new_y, new_x]))')
    print('out', val_out)
